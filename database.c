...
static char current_username[128] = {0};
...
void register_user(void) {
    ...
    if (rc != SQLITE_DONE) {
        printf("registration failed (maybe user exists)\n");
    } else {
...
int login(void) {
    ...
    sqlite3_stmt *stmt;
    const char *sql = "SELECT id,password FROM user WHERE username = ?";
    int rc = sqlite3_prepare_v2(db, sql, -1, &stmt, NULL);
...
void list_all_notes(void) {
    sqlite3_stmt *stmt;
    const char *sql =
        "SELECT n.id, u.username, n.content, n.updated_at "
...
void edit_note(void) {
    ...
    if (!fgets(buf, sizeof(buf), stdin)) return;
    buf[strcspn(buf, "\n")] = '\0';
    const char *updatesql = "UPDATE note SET content = ?, updated_at = CURRENT_TIMESTAMP WHERE id = ?";
